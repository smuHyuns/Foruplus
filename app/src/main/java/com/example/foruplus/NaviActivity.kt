...
import com.example.foruplus.databinding.ActivityNaviBinding
...
class NaviActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.navigationView.setOnItemSelectedListener { item ->
            when(item.itemId) {
                R.id.searchFragment -> setFragment(TAG_CALENDER, SearchFragment())
                R.id.homeFragment -> setFragment(TAG_HOME, HomeFragment())
                R.id.settingsFragment-> setFragment(TAG_MY_PAGE, SettingsFragment())
            }
            true
        }
    }
